...
CREATE TABLE `inversor` (
  ...
  `nombreInversor` varchar(25) NOT NULL,
  `direccionInversor` varchar(30) NOT NULL,
  `telefonoInversor` varchar(20) NOT NULL,
  `emailInversor` varchar(30) NOT NULL,
  `saldoCuenta` float DEFAULT NULL,
  `idBroker` int DEFAULT NULL,
  PRIMARY KEY (`idInversor`),
  UNIQUE KEY `telefonoInversor` (`telefonoInversor`),
  UNIQUE KEY `emailInversor` (`emailInversor`),
  KEY `idBroker` (`idBroker`),
  CONSTRAINT `inversor_ibfk_1` FOREIGN KEY (`idBroker`) REFERENCES `broker` (`idBroker`)
